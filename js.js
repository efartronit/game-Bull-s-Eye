...
document.getElementById("img_level").addEventListener("click", levelup1);

function levelup1() {
  window.open("levelup.html", "_self")
}
function homer() {
...
function checking() {
  ...
  if (flag == false) {
    ...
    if (counterblack == 4) {
      ...
      for (let index = 0; index < counterblack; index++) {
        arr_result[index].style.backgroundColor = "rgb(127 127 127)"
  ...
  localStorage.setItem("counter", counterlevel);
